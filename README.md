# An Analysis of Theatrical Fundraising Campaigns
...
We will focus on comparing kickstarters concerning interest, finances, timing, location, and other confounding factors that may influence the trajectory of a campaign. Furthermore, we will provide visual representations of the dataset, the campaigns' outcomes, and chronological and statistical analyses. 
...
4. Create a new column of data called *Percentage Funded*, and <br>
  a. Determine campaigns' funding percentages from the *pledged* and *goal* columns, and use the ROUND formula as such: `=ROUND(E2/D2*100,0)`.<br> 
...
7. Create a pivot table in a new worksheet named *Subcategory Statistics*, and choose *country* and *Parent Category* as filters; *outcomes* for columns; *Subcategory* for rows, and "outcomes* as values. Then, choose a stacked column chart type.
...
### Challenges and Solutions
  ...
  - **Average Donation** #DIV/0! Error: When scrolling through the average donations, we noticed an error: `#DIV/0!` because every campaign would require a fundraising goal but would not necessarily get any backers, so naturally, dividing by 0 would give an error result.
  ...
  - **COUNTIFS()**: When populating the function to include two criteria for the goal amount, `=COUNTIFS(Kickstarter!$D:$D, ">=1000", Kickstarter!$D:$D, "=<4999", Kickstarter!$F:$F, "successful", Kickstarter!$R:$R, "plays")`, it would result in a zero. To confirm this was an erroneous output, we manually filtered the *Kickstarter* sheet for successful campaigns with a goal amount between $1,000 and $4,999 and found 388 records. 
...
1. The preliminary inspection of the Excel file (crowfunding_StarterBook.xlsx) reveals the following: 
    ...
    - Each campaign contains information about its name and description, category and subcategory, goal and pledged funds, outcome and number of backers, launch and end dates, and country and currency.
...
2. After filtering and sorting the data based on funds and outcomes, we found that:
    -   The highest goal for a crowdfunding campaign was ($100,000,000.00). 
...
4. Many campaigns missed their goal amount by a small margin. By calculating the percentage funded for each campaign and applying a color scale from red (minimum) to blue (maximum), we could easily judge the outcome and determine how close a campaign came to reaching -and sometimes exceeding their funding goal.<br> [Crowdfunding Excel file.](Data/crowdfunding_StarterBook.xlsx)   
...
9. Louise had estimated a $10,000 budget for her play, Fever, and is interested in knowing her success chances with such a budget. Therefore, we examined the relationship between the outcomes of theatrical production kickstarters and their monetary goal amounts. <br>
    ...
    - The success rate tapered as the goal went higher until about $15,000, where failure was dominant. 
...
12. The descriptive statistical analysis of 412 successful and 250 failed campaigns supporting plays in the United States: 
    ...
    -  Louise is asking for more than twice the average successful Kickstarter goal, which is concerning for the *Fever* campaign. 
    -  The mean and median pledged amounts for failed campaigns were much lower than the successful pledges, which indicates that failed Kickstarters were unsuccessful for reasons other than asking for too much money. The high asking price would have been a culprit in the failure of a campaign if the median pledged amount for the failed projects was around $3,000. Since the median is much lower, another factor must keep people from pledging to those unsuccessful projects. 
    -  The mean of each distribution is around the third quartile, so the data follows similar distributions in each subset.
    -  The standard deviation is greater than the mean; thus, everything below the mean is considered "close" to the center. 
...
14. While Louise is committed to creating a play in the United States, she is also interested in researching musicals in Great Britain for a future project with an estimated budget of £4,000. To present Louise with the big picture, we have created a box plot for the 26 British musicals, and it shows that:
      ...
      -  Half of the campaign goals are less than £2,000, just over the third quartile for amounts pledged. <br>
...
In conclusion, the extensive crowdfunding dataset provided valuable information about the different fundraising campaigns and their path to success. <br>The main takeaways from our analysis are: 
